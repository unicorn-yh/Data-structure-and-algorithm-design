...
void backtrack(int t,int s){ 
    if(m+1-s>n+1-t) return;  //剩余皇后大于剩余行数，返回 
    if(s==m+1){                    //求出一种解 
    ...
    for(int i=1;i<=n;i++){      //已定行t，遍历i列   
        ...
        if(flag){   //这个位置没问题才放下一行 
            for(int j=1;j<=n;j++){  //计算皇后行和列的势力 
                visit[t][j]++;    //已定t行，遍历j列，更新皇后的势力 
                visit[j][i]++;    //已定i列，遍历j行，更新皇后势力 
            } 
            int rowstart=max(t+1-i,1);   //向右下的对角线 
            int columnstart=max(1,i-t+1); 
            for(int j=rowstart,k=columnstart;j<=n&&k<=n;j++,k++) 
                visit[j][k]++;          //更新皇后遍布的势力 
            rowstart=min(n,t+i-1);     //向左下的对角线 
            columnstart=max(1,i-(n-t)); 
            for(int j=rowstart,k=columnstart;j>=1&&k<=n;j--,k++) 
                visit[j][k]++;        //更新皇后遍布的势力 
            visit[t][i]-=3; 
            backtrack(t+1,s+1); 
 
            for(int j=1;j<=n;j++){    //回溯完后，还原初始状态 
                visit[t][j]--; 
                visit[j][i]--; 
            } 
            rowstart=max(t+1-i,1);   //向右下的对角线 
            columnstart=max(1,i-t+1); 
            for(int j=rowstart,k=columnstart;j<=n&&k<=n;j++,k++) 
                visit[j][k]--;          //更新皇后遍布的势力 
            rowstart=min(n,t+i-1);     //向左下的对角线 
            columnstart=max(1,i-(n-t)); 
